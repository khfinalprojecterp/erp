...
// 발주요청 화면 전환 매소드 
	 ...
	 * method=RequestMethod.GET) public String insertM_storageView(M_storage
	 * m_storage) {
	 * 
...
//발주요청 기능수행메소드
	@RequestMapping(value = "/m_storage/insertM_storage.do", method = RequestMethod.GET)
	public String insertM_storage(
			/* M_storage m_storage */

			@RequestParam int IDCODE, @RequestParam int MCODE, @RequestParam int SCODE, @RequestParam int MSMSTOCK,
			@RequestParam Date MSDATE, @RequestParam int MSPRICE, @RequestParam String MSSTATUS) {
...
//여기까진 되는데 
	...
	public String selectM_storageList(Model model, HttpSession session) {
		...
		ArrayList<Map<String, String>> slist = new ArrayList<>(m_storageService.storageDetailList(ecode));
		System.out.println(slist);
		model.addAttribute("slist", slist);

		// mlist 기자재 리스트
		ArrayList<Map<String, String>> mlist = new ArrayList<>(m_storageService.materialsDetailList(ecode));
	...
	public String updateM_storage(@RequestParam int MSNO,
			...
			@RequestParam int MSPRICE, Model model) {
